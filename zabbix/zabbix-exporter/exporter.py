...
Version: 3.0 - Fixed availability detection
"""
...
def connect_zabbix():
    ...
    try:
        ...
        logger.debug(f"Connecting to Zabbix API: {zabbix_url}")
        ...
        if api_token:
            ...
            logger.debug(f"Using user authentication: {zabbix_user}")
...
def collect_server_metrics(zapi, host):
    ...
    # OPRAVENÉ: Čítaj available z interfaces, nie z host objektu
    is_available = False
    interfaces = host.get('interfaces', [])
    if interfaces:
        # Host je dostupný ak má aspoň jeden dostupný interface
        is_available = any(iface.get('available') == '1' for iface in interfaces)
    ...
    if groups:
        ...
        for group in group_names:
            ...
            if any(x in group_lower for x in ['linux', 'centos', 'ubuntu', 'debian', 'rhel']):
                os_type = 'linux'
                break
            elif any(x in group_lower for x in ['windows', 'win']):
                os_type = 'windows'
                break
    ...
    if not is_available:
        logger.debug(f"Skipping metrics for unavailable host: {hostname}")
        return
...
def collect_all_metrics():
    ...
    try:
        ...
        try:
            # OPRAVENÉ: Pridané selectInterfaces pre správne zistenie dostupnosti
            hosts = zapi.host.get(
...
def main():
    ...
    # Configuration
    port = safe_int(os.getenv('PORT', '9285'), 9285)
    scrape_interval = safe_int(os.getenv('SCRAPE_INTERVAL', '60'), 60)
    
    # Set exporter info
    zabbix_exporter_info.info({
        'version': '3.0-fixed',
        'start_time': datetime.now().isoformat(),
        'features': 'windows-linux-server-metrics-fixed-availability'
    })
    
    # Startup message
    logger.info("=" * 70)
    logger.info("Zabbix Prometheus Exporter v3.0 (Fixed)")
    ...
    while not shutdown:
        ...
        try:
            success = collect_all_metrics()
            
            if success:
                consecutive_errors = 0
            else:
                consecutive_errors += 1
                logger.warning(f"Collection failed ({consecutive_errors}/{max_consecutive_errors})")
                
                if consecutive_errors >= max_consecutive_errors:
                    logger.error(f"Too many consecutive errors ({consecutive_errors}), but continuing...")
                    consecutive_errors = 0  # Reset to keep trying
        
        except Exception as e:
            consecutive_errors += 1
            logger.error(f"Exception in collection cycle: {e}")
            import traceback
            traceback.print_exc()
...
if __name__ == '__main__':
    try:
        ...
        logger.info("Exporter stopped by user (KeyboardInterrupt)")
        sys.exit(0)
    except Exception as e:
